...
import {
    WHITE_PIECE_COLOR, BLACK_PIECE_COLOR, WHITE_KNIGHT_NAME, BLACK_KNIGHT_NAME
} from "../src/constants.js";

test('Knights valid general movements with simple position', () => {
    const board = new Board('8/8/3N4/4n3/6n1/2N5/8/8 w - - 0 1', true);
    const moves = [
    ...
    moves.forEach(([initialSquare, targetSquare]) => {
        ...
        switch (movedPieceColor) {
            case WHITE_PIECE_COLOR:
                expect(board._getPiece(targetSquare).name).toBe(WHITE_KNIGHT_NAME);
                break;
            case BLACK_PIECE_COLOR:
                expect(board._getPiece(targetSquare).name).toBe(BLACK_KNIGHT_NAME);
                break;
        }
    });
});
...
test('Knights valid general movements with more pieces and randomly placed', () => {
    const board = new Board('1N3n1n/n5N1/3n2nN/3N4/1Nn1nN2/2N2n2/1n6/3n2N1 b - - 0 1', true);
    const moves = [
...
test('Invalid white knights moves', () => {
    const board = new Board('5N2/1N1n2nN/3N4/1n2nN2/2N3n1/3nN3/2N2nN1/8 w - - 0 1', true);
    const moves = [
...
test('Invalid black knights moves', () => {
    const board = new Board('n6n/2Nn2N1/8/2nNn3/5Nn1/2n2N2/2Nn4/n6n b - - 0 1', true);
    const moves = [
